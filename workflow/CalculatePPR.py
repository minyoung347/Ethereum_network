...
def construct_monthly_network(number):
    add_label = pd.read_csv(
        "/home/damini/Ethereum/Labeling/addresslabel.txt",
        names=["addID", "type", "nametag"],
        sep=" ",
    )
    add_label = add_label.set_index("addID")

    genesis = pd.read_csv(
        "/home/damini/Ethereum/Labeling/genesis_labels.txt",
        usecols=[0, 1],
        names=["addID", "type"],
        sep=" ",
    )
    ...
    miner = pd.read_csv(
        "/home/damini/Ethereum/Basic_Dataset/blockminer.txt",
    ...
    transaction = pd.read_csv(
        "/home/damini/Ethereum/Except_tx_fee/transaction_data_except_txfee.csv"
    )
    transaction["txdatetime"] = transaction["txtime"].map(
        lambda x: datetime.fromtimestamp(x, timezone.utc)
    )

    time_from = datetime.fromtimestamp(1438387200, timezone.utc) + relativedelta(
        months=1 * int(number)
    )
    time_until = time_from + relativedelta(months=1)
    tx_bytime = transaction[
        (transaction.txdatetime >= time_from) & (transaction.txdatetime < time_until)
    ]
...
def calculate_individual_PPR(graph, target):
    ...
    pagerank = nx.pagerank(
        graph, alpha=0.85, personalization=personal, weight="logvalue"
    )
    return pagerank
